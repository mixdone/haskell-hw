...
m_g1 f i c x | (f i x) == 0 = i
             | c > 100 = error "mostly likely the function is not defined"
             | (f i x) >= (f (i - 1) x) = m_g1 f (i + 1) (c + 1) x
             | otherwise = m_g1 f (i + 1) 0 x
func1 f = m_g1 f 0 0

m_g2:: (Integer -> Integer -> Integer -> Integer) -> Integer -> Integer -> Integer -> Integer -> Integer
m_g2 f i c x1 x2 | (f i x1 x2) == 0 = i
                 | c > 100 = error "mostly likely the function is not defined"
                 | (f i x1 x2) >= (f (i - 1) x1 x2) = m_g2 f (i + 1) (c + 1) x1 x2
                 | otherwise = m_g2 f (i + 1) 0 x1 x2
func2 f = m_g2 f 0 0
...
m_g3 p i c x | (p i x) == True = i
             | c > 1000 = error "mostly likely the function is not defined"
             | otherwise = m_g3 p (i + 1) (c+1) x
predicat p = m_g1 p 0 0
